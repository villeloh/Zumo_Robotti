...
/*******************************************************************************
* Function Name: timer_clock_5_Start
********************************************************************************
*
* Summary:
...
void timer_clock_5_Start(void) 
{
    ...
    timer_clock_5_CLKEN |= timer_clock_5_CLKEN_MASK;
	timer_clock_5_CLKSTBY |= timer_clock_5_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: timer_clock_5_Stop
********************************************************************************
*
* Summary:
...
void timer_clock_5_Stop(void) 
{
    ...
    timer_clock_5_CLKEN &= (uint8)(~timer_clock_5_CLKEN_MASK);
	timer_clock_5_CLKSTBY &= (uint8)(~timer_clock_5_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: timer_clock_5_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((timer_clock_5_CLKEN & timer_clock_5_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = timer_clock_5_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = timer_clock_5_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* timer_clock_5__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(timer_clock_5_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(timer_clock_5_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: timer_clock_5_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: timer_clock_5_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = timer_clock_5_GetSourceRegister();
    uint16 oldDivider = timer_clock_5_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = timer_clock_5_CLKEN & timer_clock_5_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(timer_clock_5_DIV_PTR, clkDivider);
                timer_clock_5_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                timer_clock_5_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(timer_clock_5_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((timer_clock_5_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: timer_clock_5_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(timer_clock_5_DIV_PTR);
}


/*******************************************************************************
* Function Name: timer_clock_5_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    timer_clock_5_MOD_SRC |= modeBitMask & (uint8)timer_clock_5_MODE_MASK;
}


/*******************************************************************************
* Function Name: timer_clock_5_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    timer_clock_5_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(timer_clock_5_MODE_MASK));
}


/*******************************************************************************
* Function Name: timer_clock_5_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return timer_clock_5_MOD_SRC & (uint8)(timer_clock_5_MODE_MASK);
}


/*******************************************************************************
* Function Name: timer_clock_5_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = timer_clock_5_GetDividerRegister();
    uint8 oldSrc = timer_clock_5_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        timer_clock_5_MOD_SRC |= CYCLK_SSS;
        timer_clock_5_MOD_SRC =
            (timer_clock_5_MOD_SRC & (uint8)(~timer_clock_5_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: timer_clock_5_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return timer_clock_5_MOD_SRC & timer_clock_5_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: timer_clock_5_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    timer_clock_5_PHASE = clkPhase & timer_clock_5_PHASE_MASK;
}


/*******************************************************************************
* Function Name: timer_clock_5_GetPhase
********************************************************************************
*
* Summary:
...
{
    return timer_clock_5_PHASE & timer_clock_5_PHASE_MASK;
}
