...
int main()
{
    ...
    // Maximum amounts of difference in measured blackness value (used in various calculations).
    ...
    // For counting black lines (to stop the robot at race end).
    int countLines = 0;
    int inBlack = 0;
    int exitMainLoop = 0;
    ...
    // Wait for IR signal. Upon getting it, proceed forward at full speed for 0.3 seconds.
    IR_val = get_IR();
    if (IR_val) 
    {
    ...
    {    
        ...
        // NOTE: due to calibration of the motor speeds, 240 (255 - 15) is our current max speed!
        
        // Start going forward.
        Custom_forward(speed);
        
        // Read initial / reaction-determining blackness value.
        ...
        // When the measured blackness value drops below the designated blackness threshold, start the turning routine.
        // (The turning direction will depend on the value of dir_flag set directly above.)
        ...
        { 
            ...
            do {
                ...
                // 'Digital' (the condition for the do-while loop) needs to be defined here, 
                // based on the new measurement (as it repeats inside the do-while loop).
                // NOTE: dir_flag is used as a condition because using the new measured dig.x value 
                // would lead to a 1 ms discontinuity between the values responsible for definitions 
                // inside and outside the turn loop. In practice this might not actually matter, 
                ...
                // Calculate the difference between the two measured black values.
                ...
                // Needed for eliminating 'freak' blackness values (explained below).
                diff_prev_ave = (diffs[2] + diffs[1]) / 2;
                
                // Limit diff_ave (needed for use in the exponential 2nd component of 'turn', to prevent potential 'freak' behaviour).
                diff_norm = diff_ave;
                if (diff_norm > 2000.0) { diff_norm = 2000.0; } // 2 000 seems like a reasonable limit, based on measured diff values
                
                // Adjust the value of blackness_2 (assigned to a new variable; original blackness_2 remains unmodified).
                // The new 'normalized' value is used instead of plain blackness_2 in the first component of 'turn', directly below.
                ...
                if ((blackDiff > 3000) && (fabsf(diff_prev_ave) < 1000) )
                {
                    ...
                    // '3 000' as a limit is speculative, as it's pretty hard to test the effect of this operation.
                }
                
                // When moving away from the line (or close enough to moving away), set turn to zero.
                if (diff_norm < -100) 
                { 
                    turn = 0; 
                    ...
                    turnComp_2 = 0.095 * ( powf( fabsf(diff_norm), 1.84 ) / maxDiff ); 
                    // NOTE: using the absolute value causes 'wrong' turn behaviour between diff -100-0, but it's worth it to 
                    // eliminate possible 'turbulence' (due to delay and measurement inaccuracy) on the borderline between negative and positive diff values.
                    ...
                    // minimum turn is about 67 atm, and this seems to give nice, smooth results.
